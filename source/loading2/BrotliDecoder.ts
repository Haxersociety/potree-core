import {POINT_ATTRIBUTES, PointAttribute, PointAttributeTypes} from "./PointAttributes";
...
export class BrotliDecoder {
    decode({pointAttributes, scale, name, min, max, size, offset, numPoints, buffer}) {
        let _buffer;
        if (numPoints === 0) {
            _buffer = {buffer: new ArrayBuffer(0)};
        } else {
            try {
                _buffer = BrotliDecode(new Int8Array(buffer));
            } catch (e) {
                _buffer = {buffer: new ArrayBuffer(numPoints * (pointAttributes.byteSize + 12))};
                console.error(`problem with node ${name}: `, e);
            }
        }
        const view = new DataView(_buffer.buffer);
        ...
        const occupancy = numPoints / numOccupiedCells;

        {
            ...
            attributeBuffers['INDICES'] = {buffer: buff, attribute: POINT_ATTRIBUTES.INDICES};
        }
